...
class CharDictionary:
    ...
    def compute_dict(dict):
        for st_content in dict['content']:
            words_and_count = collections.Counter(st_content['words'])
            for i in words_and_count:
                tf = words_and_count[i] / float(len(st_content['words']))

                e = 0.0000001
                s = sum([1.0 for st in dict['content'] if i in st['words']])
                idf = math.log10(len(dict['content']) - s + 0.5 / s + 0.5)
                if idf < e:
                    idf = e
            ...
            st_content['words'] = collections.OrderedDict(sorted(words_and_count.items(), key=lambda x: x[1]['weight'],
                                                                 reverse=True))
...
class CharDictAnalysis:
    ...
    # Получить название стандарта по коду
    def get_st_name_by_code(self, code):
        ps = self.db['prof_standarts'].find_one({'code': code})
        if not ps:
    ...
    # сравнение всех стандартов друг с другом
    def st_all_comparison(self):
        result = []
        ...
        for idx, content in enumerate(dict['content']):
            for i in range(idx + 1, len(dict['content'])):
                ...
                result.append(
                    self.ps_comparison(content[type_content_code], dict['content'][i][type_dict_content_code]))
        return result
    ...
    def ps_comparison(self, ps_one, ps_two):
        ...
        ps_one_name = self.get_st_name_by_code(ps_one)
        ps_two_name = self.get_st_name_by_code(ps_two)
    ...
    def get_percent_similarity(words_one, words_two):
        ...
        for word in words_one:
            if word in words_two:
                percent += min(words_one[word]['tf'], words_two[word]['tf'])
    ...
    def get_venn(self, ps_code_one, ps_code_two):
        ...
        for content in dict['content']:
            ...
            if 'ps_code' in content:
                ...
                code_key = 'fgos_code'
            if content[code_key] == ps_code_one or content[code_key] == ps_code_two:
                words_set = set(list(content['words']))
                sets.append(words_set)
                labels.append(content[code_key])
...
def main():
    ...
    """
    # Создание классификатора и получение отфильтрованных трудовых функций (по направлению)
    ...
    # Формирование словаря характеристик по направлению, трудовым функциям, (направлению ФГОС)
    dict = CharDictionary()
    ...
    # Преобразование словаря характеристик в коллекцию характеристик
    dict = CharDictionary()
    dict.decomposition_dict_to_collection('ped')

    # Сравнение всех стандартов друг с другом
    analysis = CharDictAnalysis('it')
    comparison = analysis.st_all_comparison()

    # Вывод рез-татов сравнения
    for comp in comparison:
        print(comp['first_ps']['code'], comp['first_ps']['name'], " | ", comp['second_ps']['code'], comp['second_ps']['name'], " | ", comp['result'])

    # Сравнение двух стандартов друг с другом и вывод рез-татов
    analysis = CharDictAnalysis('it')
    print(analysis.ps_comparison('06.003', '06.022'))

    # Визуализация анализа данных словарей характеристик
    ...
    '''
    #analysis = CharDictAnalysis('it')
    #analysis.get_venn('06.001', '06.003')
    #analysis.get_general_venn()
